...
### Added
- **Security patch — Next.js updated to close 5 known vulnerabilities** (PR #220) — updated the app's core framework to fix five security issues disclosed by Next.js on July 22, including ones that could have let an attacker bypass access rules or trick the server into contacting an address it shouldn't.
- **Android push notifications now actually send** (PR #221) — the app was recording device sign-ups for push notifications but never actually sending anything through them. It now sends real notifications to Android devices.
- **iOS push notifications no longer fail and wipe out valid device registrations** (PR #222) — iOS phones use a different notification format than Android. Sending iOS notifications through the Android system was failing, and the failure was incorrectly treated as a sign the device registration was dead, deleting it. iOS notifications are now safely skipped (not deleted) until proper iOS-specific delivery is built.
- **Vault reorganized into "Featured" and "Library" sections** (PR #223) — the top of the Vault page now always shows the 6 newest resources; a full searchable, page-by-page library sits below it.
- **Added a section label above Featured Vault content** (PR #224) — a small heading now visually separates "Featured" from "Library" so the two read as distinct sections.

## [July 25, 2026] — Content Automation Pipeline

### Added
- **New Content Automation admin tool** (PR #225) — Keith's team can now review AI-generated article drafts before anything goes live. A weekly automated job searches the web and writes draft articles across configurable topics; nothing publishes without a human clicking Publish.
- **Direction notes, reference file upload, and source visibility added to draft generation** (PR #226) — admins can now type guidance to steer what a draft focuses on, attach a reference document for that run, and see exactly which sources a generated draft cited. A draft that can't find enough credible sources is flagged instead of being generated with weak sourcing.
- **Fixed a timeout that could cut off on-demand article generation** (PR #227) — the "Generate Drafts" button now gets a full 5 minutes to finish its research-and-write process instead of timing out early.
- **Fixed the same timeout for the weekly automated run** (PR #228) — same fix as above, applied to the scheduled Monday job.
- **Full-article preview added to the draft review screen** (PR #229) — admins can now read a complete draft, not just a short preview, before deciding to publish or discard it, and can publish/discard directly from that view.
- **Scheduled publishing for drafts** (PR #230) — admins can now pick a future date and time for a draft to go live automatically instead of publishing it by hand. A background check runs every hour and publishes anything whose scheduled time has passed.

## [July 26, 2026] — Trusted Sources, Homepage Automation, RAG

### Added
- **Trusted Sources allowlist for research** (PR #231) — admins can now restrict the AI's web research to a vetted list of sources (seeded with 13 to start, covering AI & Ethics and Church & Tech topics). When at least one source is turned on, drafts are researched only from that approved list; with none turned on, research works exactly as before.
- **Improved the look of the draft preview button** (PR #232) — a small visual polish, no behavior change.
- **Homepage "Featured Insight" now updates automatically** (PR #233) — this section used to require a manual admin update. It now always shows the most recently published Vault article on its own.
- **"AI Tool of the Week" automation added** (PR #234) — the weekly automated job now also searches for a relevant AI tool to feature and lands the suggestion in a new review queue. Publishing a suggestion updates the homepage immediately; discarding removes it from the queue.
- **Fixed a security gap in the Tool of the Week link handling** (PR #235) — a web-search-sourced link could have reached the homepage without confirming it was a normal, safe web address, which could have let an unsafe link get published. Links are now checked at every step — when saved, when published, and when displayed — and rejected if they aren't a standard http/https address.
- **Homepage content now archives instead of being overwritten** (PR #236) — publishing a new "Tool of the Week" or a new "Latest Intelligence Brief" now automatically saves the outgoing one into the Vault as a browsable article, so past picks and briefs aren't lost.
- **Reminder banner for a stale Intelligence Brief** (PR #237) — the admin dashboard now shows a reminder when the Latest Intelligence Brief hasn't been updated in 30+ days.
- **On-demand button to generate a new Tool of the Week suggestion** (PR #238) — admins no longer have to wait for the weekly automated run; they can trigger a new suggestion immediately.
- **Fixed a bug that discarded good tool suggestions over a minor length overage** (PR #239) — a suggestion was being thrown out entirely if one field ran slightly over its length limit, even when the suggestion itself was good. It's now trimmed to fit instead of thrown out.
- **RAG knowledge base added for smarter content generation** (PR #240) — a new capability that lets new draft and tool-suggestion generation look up Keith's existing published content and build on it, instead of repeating it, whenever a genuinely similar topic already exists. **Status: code merged to `main` on 2026-08-02, but not yet functional in production.** It depends on two things still outstanding as of this update: (1) an API key that has not yet been added to the app's hosting environment, and (2) a database change that failed to apply because of a technical bug in its setup script (fix pending). Until both are resolved this feature has zero effect — existing publishing and draft generation continue to work exactly as before. Includes admin training documentation for the new "Backfill Embeddings" button.
