...
### Added
- **RAG knowledge base for Content Automation** — new `vault_embeddings` table (pgvector, 1536-dim, matching OpenAI's `text-embedding-3-small`) stores an embedding per published Vault article. Every new article/tool archive gets embedded on publish (non-blocking — a failed embed never blocks the publish itself). Before generating a new vault-draft article or AI Tool of the Week suggestion, the pipeline now searches for the most similar existing published content (cosine similarity, threshold 0.7) and — only when a genuine match is found — adds a "Keith's existing thinking on this topic" section to the generation system prompt, so new content builds on his established voice/positions instead of repeating them. New `POST /api/admin/content-automation/embeddings/backfill` route (+ a small "Backfill Embeddings" button on the Content Automation admin page, shown only when needed) embeds any published articles that predate this feature. **Anthropic has no native embeddings endpoint** (confirmed against their docs this session — they recommend Voyage AI or third-party providers); this uses OpenAI's `text-embedding-3-small` via raw fetch instead. **`OPENAI_API_KEY` is not currently configured in this project's Vercel environment** (verified via `vercel env list production` — absent from all existing vars) — every embedding-dependent function will fail until it's added to both Production and Preview. This is a known, flagged blocker for this PR, not an oversight; the feature degrades gracefully everywhere (publishing/generation still works, RAG lookups just return no results) until the key exists.
...
### Added
- **Latest Intelligence Brief staleness alert** — the Admin Dashboard Overview tab now shows an amber reminder banner when the Latest Intelligence Brief hasn't been updated in 30+ days ("last updated N days ago — consider publishing a new one"), with a link straight to the Content tab and a per-session dismiss. `brief_config` now tracks a server-stamped `last_updated` timestamp (set automatically on every save, not client-supplied) via a new `GET /api/admin/content-automation/brief-status` endpoint. No migration — this rides on the existing `page_configs.brief_config` jsonb column.
