...
- **Q&A release_mode fully wired** — all three modes now work: `all` shows every submitted question immediately; `single` requires per-question release via the Eye button; `hide_all` shows nothing to guests. Mode is set per session in the admin SessionManager.
