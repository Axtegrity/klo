    ...
    fetch("/api/live-events")
      ...
      .then((data) => {
        if (Array.isArray(data)) {
          // Only show events whose date is today
          setEvents(data.filter((e: FeaturedEvent) => isToday(e.event_date)));
        }
      })
      .catch(() => {});
  }, []);

  // Completely hidden when no live events — no placeholder, no container
