...
import type { AboutContent as AboutContentType } from "./_AboutContent";

/* ------------------------------------------------------------------ */
/*  Hardcoded fallback — used when DB fetch fails or migration not yet  */
/*  applied. Matches the current production content exactly.           */
/* ------------------------------------------------------------------ */

const FALLBACK: AboutContentType = {
  id: "fallback",
  hero_badge: "Technology Innovator · Speaker · Pastor",
  hero_heading: "Keith L. Odom",
  hero_tagline:
...
/* ------------------------------------------------------------------ */
/*  Server component — fetches live content, falls back on error       */
/* ------------------------------------------------------------------ */

async function getAboutContent(): Promise<AboutContentType> {
  try {
    const baseUrl =
    ...
    const json = (await res.json()) as { data?: AboutContentType };
