...
const TRAINING_SECTIONS: TrainingSection[] = [
  ...
  },
  {
    ...
    steps: [
      ...
      { icon: Paperclip, label: "Attach a Reference File (optional)", detail: "Click \"Attach reference\" to upload a PDF or Word document (max 10MB) that the AI should use as supporting background for this one generation run — a report, an article, or notes you want reflected in the draft. The attachment only applies to the single Generate click that follows, and both the Direction note and the attachment clear automatically once that run finishes successfully — so the next click always starts fresh unless you add new direction or attach a new file." },
      ...
      { icon: Pencil, label: "Edit a Draft Before Publishing", detail: "Click the \"Edit\" button on a draft card (or inside the Preview window) to fix a title, excerpt, or body wording issue in place instead of discarding and regenerating. The title, excerpt, and body become editable text fields; click \"Save\" to keep the changes or \"Cancel\" to discard them. Editing is only available while a draft is still pending or scheduled — an already-published or discarded draft can't be edited here." },
      ...
      { icon: Pencil, label: "Edit a Tool Suggestion Before Publishing", detail: "Click the \"Edit\" button on a suggestion card to fix the tool name, category, description, or \"why it matters\" wording in place before publishing. Click \"Save\" to keep the changes or \"Cancel\" to discard them. Only available while the suggestion is still pending." },
      ...
      { icon: Pencil, label: "Edit a Brief Before Publishing", detail: "Click the \"Edit\" button on a brief card (or inside the Preview window) to fix the title, excerpt, or body wording in place before publishing. Click \"Save\" to keep the changes or \"Cancel\" to discard them. Only available while the brief is still pending." },
  ...
  },
  {
    ...
    adminTab: "events",
    steps: [
      { icon: Globe, label: "KLO Website Display Settings", detail: "Admin → Events → click into a specific event → \"KLO Website Display\" section. Separate from the event's Details section (which has its own \"Save Details\" button) — KLO Website Display settings save independently via their own \"Save KLO Website Display\" button." },
      { icon: Power, label: "Show Countdown Timer (toggle)", detail: "Inside KLO Website Display. Master on/off for the numeric countdown clock on /events. When off, the clock disappears and only the event card (if selected) remains." },
      { icon: Radio, label: "Show Live Section (toggle)", detail: "Inside KLO Website Display. Hides or shows the Live events list on the public /events page." },
      { icon: Eye, label: "Show Upcoming / Past Section (toggles)", detail: "Inside KLO Website Display. Two independent switches — hide either list from the public /events page." },
      { icon: Sparkles, label: "Spotlight this event (toggle)", detail: "Inside KLO Website Display. When on, this event is pinned as the spotlight card; when off, the spotlight auto-picks the nearest upcoming event." },
...
const WORKFLOWS: Workflow[] = [
  {
    ...
    emoji: "🎯",
    steps: [
      "Admin → Events tab → click 'New Event' → enter event name and date → click 'Create & Set Up'",
      "Event opens automatically — add your live sessions in the Sessions section",
      "Create your polls in the Polls section",
  ...
  },
  {
    ...
    emoji: "🎛️",
    steps: [
      "Admin → Events tab → tap any event → scroll to Publish section",
      "Choose Countdown + Position + Auto-Pick vs. Spotlight this event",
      "Show/hide Live, Upcoming, Past sections with the three toggles",
      "For 'countdown only' view: turn all three section toggles off",
      "Display on Events Page toggle in Details section hides one event without unpublishing",
    ],
  },
  {
    title: "Going Live",
    ...
    steps: [
      ...
      "Events tab → find your event → click into it — the setup strip shows at the top",
