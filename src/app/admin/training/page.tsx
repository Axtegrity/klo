...
const TRAINING_SECTIONS: TrainingSection[] = [
  ...
  },
  {
    ...
    steps: [
      ...
      { icon: BookOpen, label: "Vault Library", detail: "Manage vault articles — set title, category, type, tier, description, author, date, and file attachments. Toggle 'Feature in Executive Feed' to surface an article on the public /feed page." },
      { icon: FileText, label: "Feed Posts", detail: "Create, edit, delete, and organize feed posts with categories, tags, and visibility controls. Feed posts appear on the public /feed alongside featured vault articles." },
  ...
  },
  {
    ...
    icon: Vote,
    description: "Create, publish, and manage events with files, AI parsing, a curated countdown+spotlight, and full visibility controls.",
    adminTab: "events-page",
    steps: [
      { icon: Radio, label: "Site Spotlight Panel", detail: "Blue card at the top of the Events Page tab — controls what appears at the top of the public /events page. Holds all countdown and spotlight-card settings in one place." },
      { icon: Power, label: "Show Countdown Timer (toggle)", detail: "Master on/off for the numeric countdown clock on /events. When off, the clock disappears and only the event card (if selected) remains." },
      { icon: LayoutGrid, label: "Event Card Position", detail: "Above Countdown / Below Countdown. Controls where the spotlight event card sits relative to the clock." },
      { icon: Sparkles, label: "Auto-Pick Next Event (toggle)", detail: "When on, the spotlight automatically follows the nearest upcoming event. Turn off and a dropdown appears — pick any specific event to feature. Save to apply." },
      { icon: Eye, label: "Show Live / Upcoming / Past Section (toggles)", detail: "Three independent switches. Hide any event list from the public /events page. Turn all three off to show only the countdown + spotlight card." },
      ...
      { icon: Pencil, label: "Edit Event (expands inline)", detail: "Click the pencil icon to open the full edit form: title, dates, location, slug, access code, timezone, hosting entity, sessions, visibility, and homepage feature toggle." },
      { icon: Users, label: "Hosting Entity", detail: "Optional text field in the edit form (e.g., \"First Baptist Church\"). Shown as \"Hosted by X\" on the public spotlight card." },
      { icon: FileStack, label: "Session Editor (up to 10)", detail: "Inside the edit form — add up to 10 session entries with name, start time, end time, and room. Shown as a list on the public spotlight card. Saves independently from the event form via its own \"Save Session\" button." },
      { icon: Globe, label: "Display on Events Page (toggle)", detail: "Per-event switch inside the edit form. When off, this event is hidden from the Live/Upcoming/Past lists but the direct URL /conference/[slug] still works. Use this to archive without unpublishing." },
    ...
    tips: [
      "Homepage feature and Events-page spotlight are SEPARATE controls. Home page = Star / \"Feature on Home Page\" toggle. Events page = Site Spotlight panel at the top.",
      "For \"countdown only\" mode on /events: keep Show Countdown Timer on, turn Show Live/Upcoming/Past Section all off. The page renders just the hero and the clock.",
      "Auto-Pick uses the nearest upcoming event that is BOTH published AND has Display on Events Page on. To spotlight a hidden event, switch Auto-Pick off and pick it manually.",
      "Session entries save independently from the main event form — use the \"Save Session\" button inside the editor, not the top-level save.",
  ...
  },
  {
    ...
    tips: [
      ...
      "Events with live sessions show a blue badge with the live session count.",
    ],
    subSections: [
      {
        id: "sessions",
        title: "Live Sessions",
        icon: Radio,
        description: "Add and manage individual live sessions within an event. Each live session gets its own polls, Q&A, and toggle.",
        steps: [
          { icon: Plus, label: "Add Live Session", detail: "Create a live session with title, description, speaker name, room location, and time" },
          { icon: Power, label: "Activate Live Session", detail: "Click the power icon on any live session row to toggle is_active — this makes the live session active for attendees. Active live sessions show green. Only the currently-running live session should be active at a time during a live event." },
          ...
          { icon: MessageSquare, label: "Q&A Toggle", detail: "Enable or disable Q&A per live session (red = off, green = on). When off, attendees cannot submit questions." },
          { icon: Eye, label: "Release Controls", detail: "\"Show All\" makes all live sessions visible, \"Single Release\" shows one at a time, \"Hide All\" hides everything from attendees" },
          { icon: FileStack, label: "End Session + Archive", detail: "End Session is on /host (sticky button at bottom), not in the Conference tab. Clicking it archives the Live Session automatically." },
        ],
        tips: ["Live sessions are displayed to attendees in the order they are created."],
      },
      {
        id: "polls",
        title: "Live Polling",
        ...
        steps: [
          ...
          { icon: Zap, label: "Deploy All", detail: "Click \"Deploy All\" (top of the poll list) to push every queued poll live simultaneously. Use this when you want all polls available at once. Use \"Deploy Live\" one at a time to control the reveal pace during the session." },
          { icon: Filter, label: "Filter by Session", detail: "Use the session dropdown to view polls for a specific session only" },
          { icon: Download, label: "Export Results", detail: "Download poll results as PDF for reporting and post-event analysis" },
        ],
        tips: [
      ...
      {
        id: "setup-strip",
        title: "Event setup strip",
        icon: LayoutGrid,
        description: "Visible at the top of every event in the Conference tab",
        steps: [
          { icon: Radio, label: "Sessions column", detail: "Shows how many sessions are added; click 'Add now' or 'Manage' to go directly to the Sessions tab" },
          { icon: BarChart3, label: "Polls column", detail: "Shows how many polls are created; click 'Add now' or 'Manage' to go directly to the Polls tab" },
          { icon: Power, label: "Go live", detail: "Always available regardless of session or poll count; warns if either is missing but lets Keith decide" },
          { icon: Monitor, label: "Once live", detail: "Strip turns green and shows 'Open war room →' which opens /host in a new tab" },
        ],
        tips: [
          "The strip resets when you switch events — each event tracks its own setup state",
          "Dismiss the strip if you prefer to navigate tabs manually — it does not affect functionality",
        ],
      },
      {
        id: "host-dashboard",
        ...
        icon: Monitor,
        description: "Your war room at /host — single scrollable screen. No tabs to switch during the show. Polls, Q&A, and Announce are all in one place.",
        steps: [
          { icon: Globe, label: "Access", detail: "Navigate to keithlodom.ai/host. Anyone with the host role can log in — no full admin account required." },
          { icon: Radio, label: "Select a session", detail: "When you open /host, you see a list of sessions for the live event. Tap any session — it goes live instantly for attendees." },
          { icon: BarChart3, label: "Polls (primary control)", detail: "Polls appear directly on the page after you activate a session. Deploy one at a time or tap Deploy All. Results update in real time." },
          { icon: MessageSquare, label: "Q&A (below polls)", detail: "Scroll down to see the Q&A section — approve or hide incoming questions. Expanded by default." },
          { icon: Megaphone, label: "Announce (tap to expand)", detail: "Tap 'ANNOUNCE' to expand the announcements section and push a message to all attendees." },
          { icon: Archive, label: "History (tap to expand)", detail: "Tap 'HISTORY' to view archived sessions with immutable snapshots — poll results, Q&A transcript, and vote counts." },
          { icon: Power, label: "End Session", detail: "The End Session button is sticky at the bottom — always visible while a session is live. Tap it when the event is done." },
        ],
        tips: ["Everything is on one screen — no tabs to switch during the show. Polls are at the top because that is what you will use most."],
      },
      {
        id: "host-duties",
        title: "Host Duties",
        icon: Users,
        description: "How to run a live event — solo or with an assistant",
        steps: [
          { icon: Radio, label: "At the podium (Keith)", detail: "Deploy polls, close polls, end session" },
          { icon: Users, label: "Backstage (assistant, if you have one)", detail: "Activate the Live Session before Keith takes the stage, approve Q&A questions, send announcements, monitor results" },
          { icon: Monitor, label: "No assistant?", detail: "Keith does all of the above from /host — everything is in one view" },
          { icon: Shield, label: "Delegation", detail: "Give your assistant the host access code before the event. They log into keithlodom.ai/host with it and see everything you see. Change the code after the event to revoke access." },
          { icon: Power, label: "Access control", detail: "The app does not restrict what an assistant can do — Keith decides the split" },
        ],
        tips: [
          "Keep /host open on your phone at the podium. Keith does not need to touch the Admin panel once the event is live.",
          "If you are solo, deploy all polls before you start speaking so you only need to tap Close and move on.",
          "Change your host access code after every event.",
        ],
      },
      {
        id: "settings",
        ...
        steps: [
          { icon: UserPlus, label: "Role Manager", detail: "Assign admin, moderator, or presenter roles to users by email. Moderators can approve Q&A questions and create and deploy polls." },
        ...
        tips: ["Moderators can approve Q&A questions and create and deploy polls, but cannot create events or manage users."],
      },
    ],
  },
  {
...
const WORKFLOWS: Workflow[] = [
  {
    ...
    emoji: "🎯",
    steps: [
      "Conference tab → click 'New Event' → enter event name and date → click 'Create & Set Up'",
      "Event opens automatically — add your live sessions in the Sessions sub-tab",
      "Create your polls in the Polls sub-tab",
      "Feature on Home Page toggle ON for homepage countdown (optional)",
      "Assign hosts in the Roles section if you have a backstage team",
      "Test with access code if required",
    ],
  },
  {
    title: "Curate the Events Page",
    emoji: "🎛️",
    steps: [
      "Admin → Events Page tab → blue Site Spotlight card",
      "Choose Countdown + Position + Auto-Pick vs. Manual",
      "Show/hide Live, Upcoming, Past sections with the three toggles",
      "For 'countdown only' view: turn all three section toggles off",
      "On individual events → Display on Events Page OFF hides one without unpublishing",
    ],
  },
  {
    title: "Going Live",
    emoji: "🔴",
    steps: [
      "Confirm your access code — find it in Admin → Events → your event → Access Code field. If you want attendees to enter a code, toggle it on when creating or editing the event.",
      "Conference tab → find your event → click into it — the setup strip shows at the top of the event",
      "Setup strip — Sessions column: add at least one session. Polls column: create your polls. You can do these in any order.",
      "When ready: click Go live → in the strip. If sessions or polls are missing, the app will warn you — you can proceed anyway.",
      "Once live: strip turns green — click Open war room → to go to /host",
      "Deploy polls one at a time or use Deploy All",
  ...
  },
  {
    ...
    steps: [
      ...
      "Toggle LIVE to OFF in the Conference tab",
