...
const TRAINING_SECTIONS: TrainingSection[] = [
  ...
  },
  {
    ...
    adminTab: "events",
    steps: [
      { icon: Radio, label: "Site Spotlight Panel", detail: "Blue card at the top of the Events tab — controls what appears at the top of the public /events page. Holds all countdown and spotlight-card settings in one place." },
  ...
  },
  {
    ...
    subSections: [
      ...
      },
      {
        ...
        icon: Monitor,
        description: "Your war room at /host — one screen for polls, Q&A, session history, and ending the live session. Open it on your phone before the event starts.",
        steps: [
          { icon: Globe, label: "Access", detail: "Navigate to keithlodom.ai/host. Anyone with the host access code can log in — no full admin account required." },
          { icon: Lock, label: "Access code (optional)", detail: "Toggle on when creating or editing an event to require attendees to enter a code before joining" },
          { icon: Radio, label: "Live Session status", detail: "The active live session is shown at the top with a pulsing LIVE badge. If no session is active yet, the Go Live button links to Admin to activate one." },
          { icon: BarChart3, label: "Polls tab", detail: "Deploy queued polls one at a time or tap Deploy All. Close a poll when you are ready to move on. Results update in real time as the audience votes." },
          { icon: MessageSquare, label: "Q&A tab", detail: "Approve or hide incoming questions before they appear to the audience. Questions arrive in real time — no refresh needed." },
          { icon: Archive, label: "History tab", detail: "View archived live sessions with immutable snapshots — poll results, Q&A transcript, and vote counts. Download PDF results from here." },
          { icon: Power, label: "End Session", detail: "The End Session button is sticky at the bottom of the screen — always visible while a session is live. Tap it to archive the live session when the event is done." },
        ],
        tips: ["Anyone with the host access code can access /host."],
      },
      {
        id: "host-duties",
        title: "Host Duties",
...
const WORKFLOWS: Workflow[] = [
  {
    ...
    steps: [
      ...
      "Fill Hosting Entity + add up to 10 session entries (name, time, room)",
      "Feature on Home Page toggle ON for homepage countdown (optional)",
      "Top of Events tab → Site Spotlight → Auto-Pick off → pick this event for /events",
      "Conference tab → select event → Add live sessions → Create polls",
  ...
  },
  {
    ...
    emoji: "🎛️",
    steps: [
      "Top of Events tab → blue Site Spotlight card",
  ...
  },
  {
    ...
    steps: [
      ...
      "On /host → History tab — view the session archive",
