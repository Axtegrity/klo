...
const TRAINING_SECTIONS: TrainingSection[] = [
  ...
  },
  {
    ...
    adminTab: "events",
    steps: [
      { icon: Globe, label: "Website Display Settings", detail: "Admin → Events → click into a specific event → \"Website Display\" section. Separate from the event's Details section (which has its own \"Save Details\" button) — Website Display settings save independently via their own \"Save Website Display\" button." },
      { icon: Power, label: "Show Countdown Timer (toggle)", detail: "Inside Website Display. Master on/off for the numeric countdown clock on /events. When off, the clock disappears and only the event card (if selected) remains." },
      { icon: Radio, label: "Show Live Section (toggle)", detail: "Inside Website Display. Hides or shows the Live events list on the public /events page." },
      { icon: Eye, label: "Show Upcoming / Past Section (toggles)", detail: "Inside Website Display. Two independent switches — hide either list from the public /events page." },
      { icon: Sparkles, label: "Spotlight this event (toggle)", detail: "Inside Website Display. When on, this event is pinned as the spotlight card; when off, the spotlight auto-picks the nearest upcoming event." },
