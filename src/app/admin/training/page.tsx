...
const TRAINING_SECTIONS: TrainingSection[] = [
  ...
  },
  {
    ...
    steps: [
      ...
      { icon: FileStack, label: "Sessions Editor (up to 10)", detail: "Inside the edit form — add up to 10 sessions with name, start time, end time, and room. Shown as a list on the public spotlight card. Saves independently from the event form via its own \"Save Sessions\" button." },
    ...
    tips: [
      ...
      "Sessions save independently from the main event form — use the \"Save Sessions\" button inside the editor, not the top-level save.",
    ...
    subSections: [
      {
        ...
        steps: [
          ...
          { icon: Power, label: "Activate Session", detail: "Click the power icon on any session row to toggle is_active — this makes the session live for attendees. Active sessions show green. Only the currently-running session should be active at a time during a live event." },
          ...
          { icon: FileStack, label: "End Session + Archive", detail: "Click \"End Session\" when a session concludes. The session is immediately archived with an immutable snapshot of all polls, Q&A responses, and vote counts. Archived sessions cannot be edited. View archives from the session list." },
        ],
      ...
      },
      {
        ...
        icon: Monitor,
        description: "A dedicated presenter remote at /host — control polls and sessions from the stage without opening the full admin.",
        steps: [
          { icon: Globe, label: "Access", detail: "Navigate to keithlodom.ai/host (or tap the Host Dashboard link in the Conference tab). Requires admin or moderator role." },
          { icon: Monitor, label: "What You See", detail: "The current active session, all queued and live polls, and controls to deploy or close them — in a clean full-screen layout optimized for a phone or tablet at the podium." },
          { icon: PlayCircle, label: "Deploy from Stage", detail: "Tap any queued poll to deploy it live, or tap \"Deploy All\" to push all polls at once — without navigating back to the full Conference tab." },
          { icon: Power, label: "Mobile-Friendly", detail: "Designed for phone or tablet use at the stage. Large tap targets, minimal UI, no distracting admin chrome. Open it on your phone before the session starts." },
        ],
        tips: ["Moderators can also access /host — assign a moderator to run polls from the back of the room while you present from the stage."],
      },
      {
        id: "settings",
        title: "Settings",
...
const WORKFLOWS: Workflow[] = [
  {
    ...
    steps: [
      ...
      "Fill Hosting Entity + add up to 10 Sessions (name, time, room)",
      ...
      "Conference tab → select event → Add sessions → Create polls",
  ...
  },
  {
    ...
    steps: [
      "Toggle LIVE to OFF",
      "Click \"End Session\" in the Conference tab — session archives automatically with immutable snapshot",
      "View session archive for poll results, Q&A transcript, and vote counts",
